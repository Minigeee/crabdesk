...
import { TicketHistoryService, type TicketHistoryEntry } from '@/lib/services/client/ticket-history.service';
...
export function useTicketHistory(ticketId: string, changeType: string = 'all') {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      return historyService.getHistory(ticketId, pageParam, pageSize, changeType);
    },
