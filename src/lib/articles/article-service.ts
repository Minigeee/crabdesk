...
export type ArticleInsert = Omit<Article, 'id' | 'created_at' | 'updated_at' | 'version'>;
export type ArticleUpdate = Partial<Omit<Article, 'id' | 'created_at' | 'updated_at' | 'version'>>;
...
export class ArticleService {
  ...
  private async createChunks(articleId: string, content: string) {
    // Generate chunks
  ...
  async createArticle(article: ArticleInsert) {
    ...
    // Then create chunks
    await this.createChunks(data.id, article.content);
  ...
  async updateArticle(id: string, updates: ArticleUpdate) {
    ...
    const { data: version } = await this.supabase
      .rpc('increment_article_version', { article_id: id });
    ...
    if (updates.content) {
      ...
      await this.createChunks(id, updates.content);
    }
