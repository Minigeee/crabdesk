...
const AlertDialogContent = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof AlertDialogPrimitive.Content> & {
    ...
    /** Use reader variables? */
    reader?: boolean;
  }
>(({ className, container, reader, ...props }, ref) => (
  <AlertDialogPortal container={container}>
    {props.withOverlay !== false && <AlertDialogOverlay />}
    <AlertDialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-[50%] top-[50%] lg:top-[40%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border border-border bg-white p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg dark:border-stone-800 dark:bg-stone-950',
        reader &&
          'bg-[var(--reader-card)] border-[var(--reader-border)] text-[var(--reader-fg)]',
        className,
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Description
    ref={ref}
    className={cn('text-md text-stone-500 dark:text-stone-400', className)}
    {...props}
  />
));
...
>(({ className, variant, ...props }, ref) => (
  <AlertDialogPrimitive.Action
    ref={ref}
    className={cn(
      buttonVariants({ variant: variant || 'destructive' }),
      className,
    )}
    {...props}
  />
