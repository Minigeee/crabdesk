...
import { findPlaceholders, replacePlaceholders, type Placeholder } from './utils';
...
export function PlaceholderEditor({
  ...
  const placeholders = React.useMemo(() => findPlaceholders(content), [content]);
  ...
  return (
    <div className={cn('space-y-4 rounded-md border bg-muted/50 p-4', className)}>
      <div className='flex items-center justify-between'>
        <h4 className='text-sm font-medium'>Fill in Placeholders</h4>
        <Button size='sm' onClick={handleApplyAll}>Apply All</Button>
      </div>
          ...
          {placeholders.map((placeholder, index) => (
            <div key={index} className='flex items-center gap-2'>
              ...
              <Input
                ...
                onChange={(e) => handleValueChange(placeholder.text, e.target.value)}
                className='h-8'
